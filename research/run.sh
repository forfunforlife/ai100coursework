...
dataset_dir=/data/forfunforlife/office-items # 数据集目录，这里是写死的，记得修改
...
# config文件
config=ssd_mobilenet_v1_officeitems.config
pipeline_config_path=$dataset_dir/$config

# 先清空输出目录，本地运行会有效果，tinymind上运行这一行没有任何效果
rm -rvf $output_dir/*

# 因为dataset里面的东西是不允许修改的，所以这里要把config文件复制一份到输出目录
cp $pipeline_config_path $output_dir
...
echo "############" $current "exporting model... #################"
# 导出模型
python ./object_detection/export_inference_graph.py --input_type tf_example --pipeline_config_path $pipeline_config_path --trained_checkpoint_prefix $train_dir/model.ckpt-$current  --output_directory $output_dir/exported_graphs
